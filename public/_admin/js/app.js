...
define(function(require) {
	...
	var App = (function() {
		...
		var assetsPath = '/_admin/';

		var pagePath = '/_admin/js/pages/';
		...
		var handleModals = function() {
			...
			$('body').on('hidden.bs.modal', '.modal', function() {
				...
				var modal = $(this)
				if(modal.attr('data-delete') == 'false') return
				$(this).remove()
			});
		 ...
		 * 假设使用bootstrap table
		 * 假设所有表单均在 portlet 中
		 * 且搜索表单带有 boootstrap-table-search class
		 * 
		 */
		var handleTableSerach = function() {
			...
			//点击搜索按钮
			$('body').on('submit', 'form.boootstrap-table-search', function(e) {
				e.preventDefault()
				var form = $(this)
				var bootstrapTableDiv = form.closest('div.bootstrap-table')

				var protlet = form.closest('div.portlet')
				if(protlet.length < 1) throw new Error('boootstrap-table-search 必须在protlet中')
				var protlet_body = protlet.find('.portlet-body')
				var err = '请输入搜索条件'
				var table = bootstrapTableDiv.find('table')
				//如果搜索数据为空，错误提示
				if(isEmpty(form) == false) {
					App.alert({
						container: protlet_body,
						place: 'prepend',
						type: 'warning',
						message: err,
					})
			...
			//点击清空按钮
			$('body').on('reset', 'form.boootstrap-table-search', function(e) {
				e.preventDefault()
				var form = $(this)
				var bootstrapTableDiv = form.closest('div.bootstrap-table')
				var table = bootstrapTableDiv.find('table')
		...
		return {
					...
					var getOption = function(_option) {
						var option = {
							...
							params: function(params) {
								var _token = page.find('input[name="_token"]').val()
			...
			bootstrapTable: function(page, config) {
				requirejs(['bootstrapTable', 'bootstrapTableExport'], function() {
					for(var key in config) {
						var _config = config[key]
						var table = _config['target']
						var option = _config['option']
				...
				var init_search = function(table) {
					var search = table.find('thead').find('tr.search')
					search.remove()
					table.on('pre-body.bs.table', function(e, arg1, arg2) {
						// ...
						console.log('pp')
						table.find('thead').append(search)
					});
				}
				...
				var getOption = function(option) {
					var _option = {
						...
						queryParams: function(query) {
							var toobar = $(this.toolbar)
							if(toobar.length < 1) return query
							var form = toobar.find('form')
							if(form.length < 1) return query
							var datas = form.serializeArray()
						...
						formatRecordsPerPage: function(pageNumber) {
							return '每页 ' + pageNumber + ' 数据'
						},
						formatShowingRows: function(pageFrom, pageTo, totalRows) {
							return '显示 ' + pageFrom + ' ~ ' + pageTo + '， 共 ' + totalRows + ' 条数据；'
						},
					...
					return $.extend(true, _option, option)
				}
			},

			badge: function(target, count, type) {
			 ...
			 * 选择地区，
			 * 当用户选择了地区，
			 * 会将地区的codes值传入input_name中
			 * 
			 * @param {Object} target
			 * @param {String} input_name
			 */
			pickArea: function(target, _input) {
				target.pickArea({
					"getVal": function() {
						console.log(new Date)
						var a = $("." + $(".pick-area-dom").val())
						var val_input = a.find('input.pick-area')
						var codes = val_input.attr('data-areacode')
						//						console.log(codes)
						if(_input) _input.val(codes)
					}
				})
